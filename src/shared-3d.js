...
window.screenJouleCapture=()=>{
 ...
 if(!(p.porosity>=0&&p.porosity<1))return {unsupported:'Porosity must be in [0, 1).'};
 return {porosity:p.porosity,length:p.length,diameter:p.width,rho:p.rho*(p.resistivityBasis==='effective'?1-p.porosity:1),k:p.k,density:p.density,cp:p.cp,mode:p.mode,command:p.command,imax:p.imax,vmax:p.vmax,pmax:p.pmax};
};
