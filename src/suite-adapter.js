    const dimension=document.documentElement.dataset.dimension,storageKey='joule.workspace.'+dimension+'.v2';let restoring=false,saveTimer;
    ...
    const showSection=name=>{for(const b of nav.querySelectorAll('[data-section]'))b.setAttribute('aria-selected',String(b.dataset.section===name));activateTab(name==='model'?(dimension==='2d'?'thermal2d':'calculator'):name);stateNote.textContent=dimension==='0d'?'Joule0D: one lumped element temperature.':'Joule2D: axisymmetric r–z model. Rectangular elements use an equivalent cylinder.';};
    ...
    if(dimension==='2d'){
      ...
      const extra=document.querySelector('#thermal2d .t2d-inputs');document.querySelector('#calculator .input-column').append(extra);extra.style.border='0';extra.style.padding='0';$('t2dMaterial')?.closest('article')?.setAttribute('hidden','');document.querySelector('#heatChart')?.parentElement.querySelector('.legend')?.remove();for(const el of extra.querySelectorAll('.t2d-controls,#t2dProgress,.section-head-row'))el.hidden=true;
    }
    // Each document has its own memory and persistent settings namespace.
    for(const event of ['input','change','click'])document.addEventListener(event,e=>{if(restoring||!e.target.closest('input,select,textarea,.mode-btn'))return;clearTimeout(saveTimer);saveTimer=setTimeout(()=>{try{localStorage.setItem(storageKey,JSON.stringify(captureSettings()));}catch{stateNote.textContent='Device storage unavailable. Export settings to keep this model.';}},250);});
    ...
    // Saved settings are restored in default-case.js, after all input controls exist.
    ...
    if(dimension==='0d'){
      ...
      if(ramp&&metrics){metrics.after(ramp);foldWorkspace(ramp,'Heating curve (lumped estimate)');}
