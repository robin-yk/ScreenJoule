...
(()=>{
 ...
 window.screenJouleShared=async (c,options={})=>{if(!options.initialPreview){for(const id of names)if(c[id]!==undefined)get(id).value=c[id];if(c.commonBoundary===undefined)get('commonBoundary').value='exposed';}display();await apply(c,options);if(!is3d)stateNote.textContent=get('commonBoundary').value==='local'?'Local reactor boundaries':'Shared thermal surroundings';};
 display();
})();
