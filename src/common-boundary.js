...
(()=>{
 ...
 panel.className='common-thermal-panel';panel.style.gridColumn='1 / -1';
 ...
 function display(){const mode=get('commonBoundary').value;get('commonExternal').hidden=mode==='local';get('wrapInputs').hidden=mode!=='insulated';if(!is3d){window.sharedConvection=mode!=='local';for(const id of ['t2dWallK','t2dEndMode']){const art=get(id)?.closest('article');(art?.closest('details.workspace-detail')||art)?.toggleAttribute('hidden',mode!=='local');}if(mode!=='local'&&get('t2dHover'))get('t2dHover').textContent='Move over a cell to inspect the calculated solid temperature.';}}
