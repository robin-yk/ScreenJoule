...
for f in ['solver.js','crosscheck.js','insulation.js']:
...
s=s.replace('  })();\n  </script>', '\n'+(root/'src/shared-suite.js').read_text()+'\n'+(root/'src/common-boundary.js').read_text()+'\n'+(root/'src/insulation-ui.js').read_text()+'\n'+(root/'src/foam-texture.js').read_text()+'\n'+(root/'src/foam-suite.js').read_text()+'\n'+(root/'src/default-case.js').read_text()+'\n  })();\n  </script>',1)
