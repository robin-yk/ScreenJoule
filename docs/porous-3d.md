...
This matches the 0D/2D explicit porous-body approximation: solid conduction area and mass scale with solid fraction, and effective k is supplied independently. A matched cylindrical comparison requires entering the same porosity, skeleton-equivalent scalar resistivity, effective k, skeleton density and Cp in each workspace; switching tabs does not transfer them.
