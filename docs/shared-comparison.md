...
The homepage starts with a 3D hollow-tube preview. The common solid cylinder below is used by the 0D/2D workspaces and for compatible cross-dimensional comparisons. On tab changes, nominal cylinder dimensions, porosity, scalar resistivity, effective thermal conductivity, skeleton density, heat capacity and supply settings transfer from the active workspace to the next. Common thermal-surroundings inputs also transfer; local electrode and reactor-boundary settings and temperature-dependent tables remain workspace-specific. A 3D-only shape stays in its own workspace when switching tabs, and the last compatible cylinder inputs are retained for 0D/2D. Porous cylinders use the approximation described in [porous-3d.md](porous-3d.md).
